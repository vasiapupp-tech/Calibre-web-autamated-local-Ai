...
def save_cover_from_url(url, book_path):
    ...
    try:
        if cli_param.allow_localhost:
            img = requests.get(url, timeout=(10, 30), allow_redirects=True, stream=True)  # ToDo: Error Handling
        elif use_advocate:
            img = cw_advocate.get(url, timeout=(10, 30), allow_redirects=True, stream=True)      # ToDo: Error Handling
        else:
...
def save_cover(img, book_path):
    ...
    if not content_type:
        # Some CDNs (e.g. cdn.litres.ru) omit Content-Type; infer it from the URL.
        content_type = mimetypes.guess_type(getattr(img, 'url', ''))[0] or ""
