"""Flibusta metadata provider (pirate library, HTML search).

Flibusta has several mirrors (flibusta.is / flibusta.site / flibusta.su).
The base URL is configurable via the META_URL class attribute.
"""
...
class Flibusta(BaseMetadataProvider, Metadata):
    __name__ = "Flibusta"
    __id__ = "flibusta"

    DESCRIPTION = "Flibusta"
    META_URL = "https://flibusta.is/"
    SEARCH_URL = "https://flibusta.is/booksearch"
    DEFAULT_LIMIT = 7
    ...
    def search(
        ...
        log.info("Searching Flibusta for: %s", query)

        response = self._get(self.SEARCH_URL, params={"ask": query, "chb": "on"})
        if response is None:
            return []
        ...
        book_urls: List[str] = []
        seen: set[str] = set()
        for a in tree.xpath('//a[contains(@href, "/b/")]/@href'):
            absolute = urljoin(self.META_URL, a)
            key = self._canonical_url(absolute)
            if key and "/b/" in key and key not in seen:
                seen.add(key)
        ...
        log.info("Flibusta search found %d results", len(results))
        return results
    ...
    def _fetch_book(
        ...
        if not authors:
            # Flibusta links the author page as /a/<id>. Use its link text.
            for a in tree.xpath('//a[contains(@href, "/a/")]//text()'):
                name = str(a).strip()
