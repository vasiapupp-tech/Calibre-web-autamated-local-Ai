"""Kniga-online.com metadata provider (online reading, HTML search)."""
...
class KnigaOnline(BaseMetadataProvider, Metadata):
    __name__ = "Kniga-Online"
    __id__ = "kniga_online"

    DESCRIPTION = "Kniga-Online"
    META_URL = "https://kniga-online.com/"
    SEARCH_URL = "https://kniga-online.com/search/"
    DEFAULT_LIMIT = 7
    ...
    def search(
        ...
        log.info("Searching Kniga-Online for: %s", query)
        ...
        book_urls: List[str] = []
        seen: set[str] = set()
        for a in tree.xpath('//a[contains(@href, "/book/") or contains(@href, "/kniga/")]/@href'):
        ...
        log.info("Kniga-Online search found %d results", len(results))
        return results
    ...
    def _fetch_book(
        ...
        if not authors:
            for a in tree.xpath('//a[contains(@href, "/author/") or contains(@href, "/authors/")]//text()'):
                name = str(a).strip()
