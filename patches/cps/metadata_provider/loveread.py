"""LoveRead.ec metadata provider (pirate library, HTML search)."""
...
class LoveRead(BaseMetadataProvider, Metadata):
    __name__ = "LoveRead"
    __id__ = "loveread"

    DESCRIPTION = "LoveRead"
    META_URL = "https://loveread.ec/"
    SEARCH_URL = "https://loveread.ec/search.php"
    DEFAULT_LIMIT = 7
    ...
    def search(
        ...
        log.info("Searching LoveRead for: %s", query)

        response = self._get(self.SEARCH_URL, params={"search": query})
        if response is None:
            return []
        ...
        seen: set[str] = set()
        for a in tree.xpath(
            '//a[contains(@href, "view_global.php") or contains(@href, "read_book.php")]/@href'
        ):
        ...
        log.info("LoveRead search found %d results", len(results))
        return results
    ...
    def _fetch_book(
        ...
        if not authors:
            for a in tree.xpath('//a[contains(@href, "/author/") or contains(@href, "author")]//text()'):
                name = str(a).strip()
