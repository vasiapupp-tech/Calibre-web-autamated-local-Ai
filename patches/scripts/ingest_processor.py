...
class NewBookProcessor:
    def __init__(self, filepath: str):
        ...
        # DJVU support: convert to PDF first (djvulibre's ddjvu), then process as a normal PDF
        if self.input_format == 'djvu':
            self._convert_djvu_to_pdf()
    ...
    def _convert_djvu_to_pdf(self) -> bool:
        """Convert the current DJVU file to PDF using djvulibre's ddjvu.

        Updates self.filepath / self.filename / self.input_format in place so the
        rest of the ingest pipeline treats the book as a PDF. Scanned DJVU books
        are stored as PDF only: the epub conversion is skipped because there is no
        text layer and it would hang.
        """
        djvu_path = self.filepath
        ...
        try:
            ...
            print(f"[ingest-processor]: ERROR - ddjvu raised: {e}", flush=True)
            return False

        if result.returncode != 0:
            print(f"[ingest-processor]: ERROR - ddjvu failed: {result.stderr[-500:]}", flush=True)
            return False
        if not os.path.isfile(pdf_path) or os.path.getsize(pdf_path) == 0:
            print(f"[ingest-processor]: ERROR - ddjvu produced no output: {pdf_path}", flush=True)
            return False

        self.filepath = pdf_path
        self.filename = os.path.basename(pdf_path)
        self.input_format = 'pdf'
        self.can_convert, _ = self.can_convert_check()
        self.is_target_format = (self.input_format.lower() == str(self.target_format).lower())
        # Scanned DJVU books are stored as PDF only (skip the epub conversion)
        if 'pdf' not in self.convert_ignored_formats:
            self.convert_ignored_formats.append('pdf')
        # Remove the original DJVU so it is not re-processed (the PDF replaces it)
        try:
            os.remove(djvu_path)
        except OSError:
            pass
        print(f"[ingest-processor]: DJVU -> PDF complete: {self.filename} ({os.path.getsize(pdf_path)} bytes)", flush=True)
        return True
    ...
    def add_book_to_library(self, book_path:str, text: bool=True, format: str="text", fetch_metadata: bool=True ) -> None:
...
def main(filepath=None):
    ...
    try:
        ...
        # DJVU may have been converted to PDF inside __init__; use the converted path
        ...
        if nbp.is_target_format: # File can just be imported
            ...
            if nbp.auto_convert_on and nbp.can_convert: # File can be converted to target format and Auto-Converter is on
                ...
                if convert_successful: # If previous conversion process was successful, remove tmp files and import into library
                    # Whether the original format should be retained as an extra format.
                    retain_original = (nbp.input_format in nbp.convert_retained_formats
                                       and nbp.input_format not in nbp.ingest_ignored_formats)

                    if retain_original:
                        # Add the target format first but defer metadata so the
                        # AI cover detection can render the retained original (PDF),
                        # which is added below, before metadata runs.
                        nbp.add_book_to_library(converted_filepath, fetch_metadata=False) # type: ignore
                        ...
                        # Now fetch metadata (AI + online providers) with the retained PDF available.
                        if target_book_id is not None:
                            nbp.fetch_metadata_if_enabled(book_id=int(target_book_id))
                        else:
                            nbp.fetch_metadata_if_enabled(Path(nbp.filename).stem)
                    else:
